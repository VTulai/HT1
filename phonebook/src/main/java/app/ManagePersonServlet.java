...
public class ManagePersonServlet extends HttpServlet {
	...
	private String validatePersonPhoneNumber(String number)
	{
		String error_message = "";
		if (!new Person().validatePhoneNumber(number))
		{
			error_message += "Неправильно введённый номер телефона.Допускаются один символ #,+,-, количество цифр от 2 до 50";
		}
		return error_message;
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		RequestDispatcher dispatcher_for_person = request.getRequestDispatcher("/AddPerson.jsp");
		RequestDispatcher dispatcher_for_add_phone = request.getRequestDispatcher("/AddNumber.jsp");
		RequestDispatcher dispatcher_for_edit_phone = request.getRequestDispatcher("/EditNumber.jsp");
        ...
        else
        {
        	switch (action)
        	{
				...
				case "add_phone":
				{
					Person phone_for_person = this.phonebook.getPerson(id);
					...
					// Установка параметров JSP.
					request.setAttribute("person", phone_for_person);
					request.setAttribute("jsp_parameters", jsp_parameters);
					// Передача запроса в JSP.
					dispatcher_for_add_phone.forward(request,response);
					break;
				}
				case "edit_phone":
				{
					Person phone_edit_for_person = this.phonebook.getPerson(id);
					...
					request.setAttribute("person",phone_edit_for_person);
					request.setAttribute("jsp_parameters",jsp_parameters);
				...
				case "delete_phone":
				{
					Person person_delete_phone = phonebook.getPerson(id);
					// Если запись удалось удалить...
					if (phonebook.deletePerson(id))
					{
						jsp_parameters.put("current_action_result", "DELETION_SUCCESS");
					...
					// Установка параметров JSP.
					request.setAttribute("person", person_delete_phone);
					request.setAttribute("jsp_parameters", jsp_parameters);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (add_phone_go != null)
		{
			...
			// Валидация ФИО.
			String error_message = this.validatePersonPhoneNumber(phone);

			// Если данные верные, можно производить добавление.
			if (error_message.equals(""))
			{
				Person new_person = this.phonebook.getPerson(id);
				...
				if (this.phonebook.addPhone(new_person,phone))
				{
		...
		if (edit_phone_go != null) {
			...
			// Валидация ФИО.
			String error_message = this.validatePersonPhoneNumber(request.getParameter("phone_number"));
			// Подготовка параметров для JSP.
			jsp_parameters.put("current_action", "edit");
			jsp_parameters.put("next_action", "edit_go");
			jsp_parameters.put("next_action_label", "Сохранить");
			...
			if (error_message.equals("")) {
				...
				if (this.phonebook.updatePhone(request.getParameter("phone_number"), updatable_person)) {
