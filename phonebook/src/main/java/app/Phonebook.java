...
public class Phonebook {
	...
	{
		Integer id_person  = Integer.parseInt(person.getId());
		String query;
		query = "INSERT INTO `phone`(`owner`,`number`) VALUES ("+ id_person + ",'" + number +"')";
		Integer affected_rows = this.db.changeDBData(query);
		if (affected_rows > 0)
		{
			String phone = this.db.changeDBData(query).toString();
			// Добавляем запись о человеке в общий список.
			person.getPhones().put(phone,number);
	...
	public boolean updatePhone(String number , Person person)
	{
		Integer id_person  = Integer.parseInt(person.getId());
		String query;
		query = "UPDATE `phone` SET `number` = '"+ number+"' where owner = "+ person.getId();
		Integer affected_rows = this.db.changeDBData(query);
		if (affected_rows > 0)
		{
			String phone = this.db.changeDBData(query).toString();
	...
	public boolean deletePhone(String id)
	{
		if ((id != null)&&(!id.equals("null")))
		{
			...
			Integer affected_rows = this.db.changeDBData("DELETE FROM `phone` WHERE `owner`=" + filtered_id);
